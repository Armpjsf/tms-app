...
interface PaymentItem {
  ...
  Extra_Cost_Driver: number
  Payment_Status: string
}

// Mock data - will be replaced with real Supabase data
const mockPaymentData: PaymentItem[] = [
  { Job_ID: "JOB-001", Driver_Name: "สมชาย ใจดี", Vehicle_Plate: "1กก-1234", Plan_Date: "2026-02-01", Route_Name: "กรุงเทพ - ชลบุรี", Cost_Driver_Total: 2000, Extra_Cost_Driver: 300, Payment_Status: "รอจ่าย" },
  { Job_ID: "JOB-002", Driver_Name: "สมชาย ใจดี", Vehicle_Plate: "1กก-1234", Plan_Date: "2026-02-03", Route_Name: "กรุงเทพ - ระยอง", Cost_Driver_Total: 2500, Extra_Cost_Driver: 200, Payment_Status: "รอจ่าย" },
  { Job_ID: "JOB-003", Driver_Name: "สมหญิง รักงาน", Vehicle_Plate: "2ขข-5678", Plan_Date: "2026-02-04", Route_Name: "กรุงเทพ - อยุธยา", Cost_Driver_Total: 1500, Extra_Cost_Driver: 0, Payment_Status: "รอจ่าย" },
  { Job_ID: "JOB-004", Driver_Name: "สมหญิง รักงาน", Vehicle_Plate: "2ขข-5678", Plan_Date: "2026-02-05", Route_Name: "กรุงเทพ - นครปฐม", Cost_Driver_Total: 1200, Extra_Cost_Driver: 100, Payment_Status: "จ่ายแล้ว" },
]
...
export default function DriverPaymentPage() {
  ...
  // Get unique drivers
  const drivers = [...new Set(mockPaymentData.map(item => item.Driver_Name))]
  ...
  // Calculate totals
  const pendingItems = filteredData.filter(i => i.Payment_Status === "รอจ่าย")
  const pendingTotal = pendingItems.reduce((sum, i) => sum + i.Cost_Driver_Total + i.Extra_Cost_Driver, 0)
  
  // Selected items calculations
  const selectedData = filteredData.filter(i => selectedItems.includes(i.Job_ID))
  const selectedSubtotal = selectedData.reduce((sum, i) => sum + i.Cost_Driver_Total + i.Extra_Cost_Driver, 0)
                  ...
                  <tr key={item.Job_ID} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    <td className="py-3 px-4">
                      <input
                        ...
                        disabled={item.Payment_Status !== "รอจ่าย"}
                      />
                    </td>
                    ...
                    <td className="py-3 px-4 text-right text-indigo-400">
                      ฿{item.Cost_Driver_Total.toLocaleString()}
                    </td>
                    <td className="py-3 px-4 text-right text-amber-400">
                      {item.Extra_Cost_Driver > 0 ? `฿${item.Extra_Cost_Driver.toLocaleString()}` : '-'}
                    </td>
                    <td className="py-3 px-4 text-right text-white font-medium">
                      ฿{(item.Cost_Driver_Total + item.Extra_Cost_Driver).toLocaleString()}
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        item.Payment_Status === "รอจ่าย"
