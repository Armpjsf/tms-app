...
export default async function DriverJobsPage(props: Props) {
  ...
  return (
    <div className="min-h-screen bg-background pb-32 pt-24 px-6 relative overflow-hidden">
      ...
      <MobileHeader title="Mission Log" rightElement={<MobileJobFilter />} />
      
      <div className="relative z-10 space-y-6">
        {/* Active Title Section */}
        <div className="flex justify-between items-end px-1">
            <div className="space-y-1">
                <h2 className="text-4xl font-black text-white tracking-tighter">Your Drops</h2>
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">Fleet Deployment Schedule</p>
            </div>
            <div className="text-right">
                <div className="text-2xl font-black text-primary leading-none">{jobs.length}</div>
                <div className="text-[8px] font-black text-primary uppercase tracking-widest">Active</div>
            </div>
        </div>
        ...
        <div className="space-y-6">
            {jobs.length === 0 ? (
                <div className="text-center py-24 glass-panel rounded-[3rem] border-dashed border-white/10">
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">No deployments found</p>
                </div>
            ) : jobs.map((job) => (
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-6">
                        <div className="space-y-1">
                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                                <MapPin size={10} className="text-accent" /> Drop Point
                            </p>
                        ...
                        <div className="space-y-1 text-right">
                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center justify-end gap-1.5">
                                Scheduled <Clock size={10} className="text-primary" />
                            </p>
