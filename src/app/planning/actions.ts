...
export async function deleteJobMedia(
    jobId: string, 
    field: 'Photo_Proof_Url' | 'Signature_Url' | 'Pickup_Photo_Url' | 'Pickup_Signature_Url', 
    urlToDelete: string
) {
    const isAdminUser = await isAdmin()
    ...
    try {
        ...
        if (fetchError || !job) return { success: false, message: 'Job not found' }

        const currentVal = job[field] as string || ""
        let updatedVal: string | null = null

        if (field === 'Photo_Proof_Url' || field === 'Pickup_Photo_Url') {
            // List-based fields
            const currentUrls = currentVal.split(',').filter(Boolean)
            const filteredUrls = currentUrls.filter(url => url.trim() !== urlToDelete.trim())
            updatedVal = filteredUrls.length > 0 ? filteredUrls.join(',') : null
        } else {
            // Single-value fields (Signatures)
            updatedVal = null
        }
        ...
        if (updateError) return { success: false, message: 'Failed to update database: ' + updateError.message }
        ...
        console.error('[deleteJobMedia ERROR]', e)
