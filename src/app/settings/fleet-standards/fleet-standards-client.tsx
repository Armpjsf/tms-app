...
export function FleetStandardsClient({ initialFuel, initialMaintenance }: { initialFuel: any[], initialMaintenance: any[] }) {
    ...
    const handleSaveFuel = async (index: number) => {
        ...
        if (result.success) toast.success("บันทึกเกณฑ์น้ำมันเรียบร้อย")
        ...
        <div className="space-y-12 pb-20">
            ...
            <Tabs defaultValue="fuel" className="w-full">
                <div className="flex justify-center mb-12">
                    <TabsList className="bg-muted/50 p-2 rounded-[2rem] border border-border/5 h-auto gap-2">
                ...
                <TabsContent value="fuel" className="animate-in fade-in zoom-in-95 duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
                        {fuelData.map((std, idx) => (
                            <Card key={std.Vehicle_Type} className="bg-card border-border/10 rounded-[3rem] shadow-2xl overflow-hidden hover:scale-[1.02] transition-transform duration-500 relative">
                                ...
                                <CardContent className="p-8 space-y-8">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground bg-muted/50 px-3 py-1 rounded-full">Automated Insight</span>
                                    </div>

                                    <div>
                                        <h3 className="text-3xl font-black text-foreground tracking-tighter uppercase">{std.Vehicle_Type}</h3>
                                        <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Performance Matrix</p>
                                    </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[1000px]">
                            ...
                            <tbody className="divide-y divide-white/5">
                                {maintData.map((std, idx) => (
                                    <tr key={std.Component_Name} className="group hover:bg-primary/[0.02] transition-colors">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex items-center justify-center gap-3">
                                                <button onClick={() => handleSaveMaint(idx)} className="p-3 bg-emerald-500/10 text-emerald-500 rounded-xl hover:bg-emerald-500 hover:text-white transition-all shadow-lg">
                                                    <Save size={18} />
                                                </button>
                                                <button 
                                                    onClick={async () => {
