    ...
    <div className="space-y-10">
      {/* Sub-Section Header */}
          ...
          <PremiumCard className="bg-slate-950 border-none shadow-2xl relative overflow-hidden group p-8 rounded-br-[3rem] rounded-tl-[1.5rem]">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="space-y-1">
                  <span className="text-orange-400 text-base font-bold font-black uppercase tracking-[0.2em] italic">{t('dashboard.energy_expenditure')}</span>
                  <p className="text-base font-bold text-slate-500 font-bold uppercase tracking-widest italic">{t('dashboard.operational_fuel_matrix')}</p>
                </div>
        ...
        <PremiumCard className="bg-slate-950 border-none shadow-2xl relative overflow-hidden group p-8 rounded-br-[3rem] rounded-tl-[1.5rem]">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="space-y-1">
                <span className="text-yellow-400 text-base font-bold font-black uppercase tracking-[0.2em] italic">{t('fuel.unit_market_cost')}</span>
                <p className="text-base font-bold text-slate-500 font-bold uppercase tracking-widest italic">{t('fuel.weighted_average')}</p>
              </div>
        ...
        <PremiumCard className={cn(
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="space-y-1">
                <span className={cn("text-base font-bold font-black uppercase tracking-[0.2em] italic", anomalies.length > 0 ? "text-red-100" : "text-slate-400")}>
                    {t('fuel.integrity_alerts')}
                </span>
                <p className={cn("text-base font-bold font-bold uppercase tracking-widest italic", anomalies.length > 0 ? "text-red-200" : "text-slate-500")}>
            ...
            <div className="flex items-center gap-2 mt-4 relative z-10">
                <p className={cn("text-base font-bold font-black uppercase tracking-widest italic", anomalies.length > 0 ? "text-white animate-pulse" : "text-slate-600")}>
        ...
        <PremiumCard className="bg-white/5 border border-white/10 shadow-2xl p-0 overflow-hidden rounded-br-[5rem] rounded-tl-[3rem]">
           <div className="p-8 border-b border-white/5 bg-slate-950 relative overflow-hidden flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 relative z-10">
                ...
                <div>
                  <h3 className="text-lg font-black text-white tracking-tight italic">{t('fuel.temporal_burn_registry')}</h3>
                ...
                {monthlyTrends.map((m) => (
                    <div key={m.month} className="flex-1 flex flex-col items-center gap-4 group relative z-10">
                        <div className="w-full bg-white/5 rounded-2xl relative flex items-end justify-center group-hover:bg-white/10 transition-all duration-500 p-1" style={{ height: '100%' }}>
                            ...
                            >
                                <div className="absolute -top-10 left-1/2 -translate-x-1/2 text-base font-bold font-black text-white bg-slate-950 px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-500 whitespace-nowrap border border-slate-800 shadow-2xl scale-75 group-hover:scale-100 italic">
                                    ฿{m.totalCost.toLocaleString()}
                                </div>
                            </div>
                        </div>
                        <span className="text-base font-bold font-black text-slate-400 uppercase tracking-widest italic">{m.month}</span>
                    </div>
                ))}
        ...
        <PremiumCard className="bg-white/5 border border-white/10 shadow-2xl p-0 overflow-hidden rounded-br-[5rem] rounded-tl-[3rem]">
           <div className="p-8 border-b border-white/5 bg-slate-950 relative overflow-hidden flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 relative z-10">
                ...
                <div>
                  <h3 className="text-lg font-black text-white tracking-tight italic">{t('fuel.consumption_matrix')}</h3>
                ...
                {vehicleBreakdown.slice(0, 6).map((v) => (
                 ...
                 {vehicleBreakdown.length === 0 && (
                    <div className="p-24 text-center">
                        ...
                        <p className="text-base font-bold font-black text-slate-400 uppercase tracking-widest italic">{t('fuel.asset_register_empty')}</p>
                    </div>
                )}
              </div>
           </div>
