...
const GrowthIndicator = ({ value, isPoints = false }: { value: number, isPoints?: boolean }) => {
  ...
  return (
    <div className={`flex items-center gap-1 text-xs font-black ${isPositive ? 'text-emerald-700' : 'text-red-700'}`}>
...
export function FinancialSummaryCards({ data }: { data: ExecutiveKPIs }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <Card className="bg-card border-border shadow-sm hover:border-emerald-500/30 transition-all">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-muted-foreground font-medium">รวมสถานะ: Completed, Delivered, Finished, Closed</p>
          </div>
      ...
      <Card className="bg-card border-border shadow-sm hover:border-blue-500/30 transition-all">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-muted-foreground font-medium">รายรับ - (ค่าน้ำมัน + ค่าซ่อม + ค่าคนขับ)</p>
          </div>
          <div className="p-2 bg-blue-500/10 rounded-lg">
            <TrendingUp className="h-4 w-4 text-emerald-600 dark:text-emerald-600" />
          </div>
        </CardHeader>
        <CardContent>
          <div className="flex items-baseline gap-2">
      ...
      <Card className="bg-white border-gray-200 shadow-2xl relative overflow-hidden group">
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-sm font-black text-emerald-700 tracking-widest uppercase">Health Score</CardTitle>
          <div className="h-2 w-2 rounded-full bg-emerald-600 animate-pulse"></div>
        </CardHeader>
        <CardContent>
          <div className="text-4xl font-black text-gray-900 tracking-tighter">GOOD</div>
          <p className="text-[10px] text-gray-700 mt-1 uppercase font-black tracking-tight">Operations are within targets</p>
