...
from config.constants import DriverStatus, JobStatus, PaymentStatus
...
def render_dashboard_view():
    ...
    job_cols = "Job_ID,Job_Status,Plan_Date,Price_Cust_Total,Cost_Driver_Total,Driver_Name,Customer_Name,Payment_Status,Billing_Status,Driver_ID"
...
def _render_interactive_content(jobs):
    ...
    with col_f3:
        quick_filter = st.selectbox(
            get_label("quick_filter"), ["Custom", "Today", "This Week", "This Month", "Last 30 Days"], key="dash_filter"
        )
    ...
    html += render_metric_card(
        get_label("total_jobs"),
        f"{total_jobs}",
        icon="📦",
        trend=None,
        sub=f"{completed_jobs} {get_label('completed')}",
    )
    html += render_metric_card(get_label("revenue"), f"฿{revenue:,.0f}", icon="💰", accent_color="accent-red")
    html += render_metric_card(
        get_label("profit"), f"฿{gross_profit:,.0f}", icon="📈", trend=f"{margin:.1f}%", accent_color="accent-green"
    )
    ...
    with exp1:
        st.markdown(
            render_metric_card(get_label("driver_cost"), f"฿{driver_cost:,.0f}", icon="💵"), unsafe_allow_html=True
        )
    with exp2:
        st.markdown(
            render_metric_card(get_label("fuel_cost"), "N/A", icon="⛽", sub="Detail in Reports"),
            unsafe_allow_html=True,
        )  # Optimized out
    with exp3:
        st.markdown(
            render_metric_card(get_label("total_expense"), f"฿{driver_cost:,.0f}", icon="📉", sub="Driver Only"),
            unsafe_allow_html=True,
        )
    with exp4:
        expense_ratio = (driver_cost / revenue * 100) if revenue > 0 else 0
        st.markdown(
            render_metric_card(get_label("expense_ratio"), f"{expense_ratio:.1f}%", icon="📊"), unsafe_allow_html=True
        )
    ...
    with col_chart1:
        ...
        fig_water = go.Figure(
            go.Waterfall(
                ...
                text=[
                    f"{revenue/1000:.1f}k",
                    f"-{driver_cost/1000:.1f}k",
                    f"-{fuel_cost/1000:.1f}k",
                    f"{net_profit/1000:.1f}k",
                ],
    ...
    with col_chart2:
        ...
        if not view.empty:
            daily = view.groupby(view["PD"].dt.date).agg({"Price_Cust_Total": "sum", "Job_ID": "count"}).reset_index()
            daily.columns = ["Date", "Revenue", "Jobs"]

            fig_trend = px.area(daily, x="Date", y="Revenue", color_discrete_sequence=["#339af0"])
    ...
    with col_fleet2:
        ...
        if not drivers.empty:
            ...
            if "Current_Mileage" in drivers.columns and "Next_Service_Mileage" in drivers.columns:
                drivers["KM_Until_Service"] = pd.to_numeric(
                    drivers["Next_Service_Mileage"], errors="coerce"
    ...
    with col_top1:
        ...
        if not view.empty and "Driver_Name" in view.columns:
            driver_perf = (
                view[view["Job_Status"] == JobStatus.COMPLETED]
                .groupby("Driver_Name")
                .agg({"Job_ID": "count", "Cost_Driver_Total": "sum"})
                .reset_index()
            )
    ...
    with col_top2:
        ...
        if not view.empty and "Customer_Name" in view.columns:
            cust_perf = view.groupby("Customer_Name").agg({"Job_ID": "count", "Price_Cust_Total": "sum"}).reset_index()
