...
def _render_live_map(drivers):
    ...
    with col_map:
        if not gps_df.empty:
            ...
            # Icon Layer (รถสีแดง/เขียว) - ใช้ไฟล์รูปจริง
            import base64
            import os

            # Get absolute paths to images
            app_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            green_truck_path = os.path.join(app_dir, "รถเขียว.jpg")
            red_truck_path = os.path.join(app_dir, "รถแดง.png")

            # Convert to data URLs for pydeck
            ...
            green_url = image_to_data_url(green_truck_path)
            red_url = image_to_data_url(red_truck_path)

            # Create icon_data column with different images based on speed
            gps_df["icon_data"] = gps_df["Speed"].apply(
                lambda x: {
                    "url": green_url if x > 0 else red_url,
                    "width": 128,
                    "height": 128,
                    "anchorY": 64,
                }
            ...
            # Text Layer (แสดงทะเบียนรถ - สีดำ)
            text_layer = pdk.Layer(
                "TextLayer",
                gps_df,
                get_position="[lon, lat]",
                get_text="Vehicle",
                get_size=18,  # เพิ่มขนาดให้ใหญ่ขึ้น
                get_color=[0, 0, 0, 255],  # สีดำ
                get_angle=0,
                get_text_anchor="'middle'",
                get_alignment_baseline="'bottom'",  # แสดงด้านล่างของจุด
                get_pixel_offset=[0, 25],  # เลื่อนลงไปอีกนิด (เพิ่มจาก 15 เป็น 25)
            )
            ...
            st.pydeck_chart(
                pdk.Deck(
                    ...
                    layers=[icon_layer, text_layer],  # ใช้ icon_layer แทน scatter
    ...
    with col_info:
        ...
        if selected_row is not None:
            ...
            st.markdown("---")
            google_maps_url = f"https://www.google.com/maps?q={selected_row['lat']},{selected_row['lon']}"
            st.markdown(
