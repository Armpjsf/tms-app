...
from utils.helpers import render_metric_card, safe_float
...
VEHICLE_VOLUME = {"4W": 10, "6W": 30, "10W": 50, "Trailer": 80}  # Small truck/Pickup  # 6-Wheel  # 10-Wheel  # Trailer
...
def get_price_suggestion(customer: str, origin: str, dest: str) -> float:
    ...
    similar = jobs[
        (jobs["Customer_Name"] == customer) & ((jobs["Origin_Location"] == origin) | (jobs["Dest_Location"] == dest))
    ]
...
def render_planning_view():
    ...
    t1, t2, t3, t4 = st.tabs(
        [get_label("tab_create"), "🗺️ วางแผนเส้นทาง Multi-Drop", get_label("tab_impex"), "✨ Auto Assign"]
    )
...
def _render_create_job():
        ...
        with st.expander("📋 คัดลอกจากงานเก่า", expanded=False):
            ...
            if not recent_jobs.empty:
                for idx, (_, job) in enumerate(recent_jobs.iterrows()):
                    job_info = (
                        f"{job.get('Job_ID', '')} | {job.get('Customer_Name', '')[:15]} | {job.get('Plan_Date', '')}"
                    )
    ...
    cust_opts = (
        customers["Customer_Name"].dropna().unique().tolist()
    ...
    branch_opts = (
        users["Branch_ID"].dropna().unique().tolist() if not users.empty and "Branch_ID" in users.columns else ["HEAD"]
    )
    if not branch_opts:
        branch_opts = ["HEAD"]
    ...
    with c2:
        ...
        customer = st.text_input(get_label("customer"), key="txt_customer", placeholder="ชื่อลูกค้า (พิมพ์เองหรือเลือก)")
    ...
    if "drop_data" in st.session_state and not st.session_state.drop_data.empty:
        ...
        if not origins.empty:
            def_origin = origins.iloc[0]["Location"]
        if not dests.empty:
            def_dest = dests.iloc[0]["Location"]

    # Separate Location Lists for Origin and Destination
    # Origin List - จาก Origin column เท่านั้น
    origin_options = set()
    if not routes.empty and "Origin" in routes.columns:
        origin_options.update(routes["Origin"].dropna().unique())
    origin_list = sorted(list(origin_options))
    origin_list.insert(0, "")  # Blank option

    # Destination List - จาก Destination column เท่านั้น
    dest_options = set()
    if not routes.empty and "Destination" in routes.columns:
        dest_options.update(routes["Destination"].dropna().unique())
    dest_list = sorted(list(dest_options))
    dest_list.insert(0, "")  # Blank option

    col_r1, col_r2, col_r3 = st.columns([2, 2, 1])
    with col_r1:
        # Origin - Enhanced Searchable Dropdown
        st.markdown(f"📍 **{get_label('origin')}**")

        # Primary: Searchable Selectbox (more prominent)
        selected_origin = st.selectbox(
            "เลือกหรือพิมพ์ค้นหาต้นทาง",
            origin_list,
            index=0 if not def_origin else (origin_list.index(def_origin) if def_origin in origin_list else 0),
            key="sel_origin_dropdown",
            placeholder="พิมพ์ค้นหาหรือเลือก...",
            help="💡 พิมพ์คำค้นหาเพื่อกรองรายการ หรือเลือกจากประวัติ",
        )

        # Secondary: Manual input (for custom locations)
        # Use selected value from dropdown, or allow manual override
        origin = st.text_input(
            "หรือพิมพ์ที่อยู่เอง",
            value=selected_origin if selected_origin else "",
            key="manual_origin_input",
            placeholder="กรณีไม่มีในรายการ พิมพ์เอง...",
        )

    with col_r2:
        # Destination - Enhanced Searchable Dropdown
        st.markdown(f"🏁 **{get_label('dest')}**")

        # Primary: Searchable Selectbox
        selected_dest = st.selectbox(
            "เลือกหรือพิมพ์ค้นหาปลายทาง",
            dest_list,
            index=0 if not def_dest else (dest_list.index(def_dest) if def_dest in dest_list else 0),
            key="sel_dest_dropdown",
            placeholder="พิมพ์ค้นหาหรือเลือก...",
            help="💡 พิมพ์คำค้นหาเพื่อกรองรายการ หรือเลือกจากประวัติ",
        )
        ...
        dest = st.text_input(
            "หรือพิมพ์ที่อยู่เอง",
            value=selected_dest if selected_dest else "",
            key="manual_dest_input",
            placeholder="กรณีไม่มีในรายการ พิมพ์เอง...",
        )
    ...
    with col_r3:
        ...
        # Auto-fill distance from routes data
        auto_distance = 0.0

        # Get actual origin/dest values (prefer manual input if filled, otherwise use dropdown)
        current_origin = origin if origin else selected_origin
        current_dest = dest if dest else selected_dest

        # Search for matching route in routes data
        if current_origin and current_dest and not routes.empty:
            matching_route = routes[
                (routes["Origin"].str.strip() == current_origin.strip())
                & (routes["Destination"].str.strip() == current_dest.strip())
            ]

            if not matching_route.empty and "Distance_KM" in matching_route.columns:
                auto_distance = safe_float(matching_route.iloc[0]["Distance_KM"])

        # Use auto distance if found, otherwise use default
        final_distance = auto_distance if auto_distance > 0 else def_dist

        dist = st.number_input(
            get_label("distance"),
            value=final_distance,
            label_visibility="collapsed",
            help="ระยะทางจะถูกกรอกอัตโนมัติจากข้อมูลเส้นทาง",
        )
    ...
    with d4:
        ...
        st.markdown(
            render_metric_card(
                get_label("profit"), f"฿{profit:,.0f}", icon="💰", trend=f"{margin:.1f}%", accent_color="accent-green"
            ),
    ...
    with info_col3:
        ...
        if suggested > 0:
            ...
            st.markdown(
                render_metric_card("💡 ราคาแนะนำ", f"฿{suggested:,.0f}", icon="💡", trend=trend), unsafe_allow_html=True
            )
        else:
    ...
    with col_btn1:
        ...
        if submitted:
            ...
            new_job = {
                ...
                # "Total_Volume_cbm": cbm, # Column not found in DB
            ...
            try:
                # Use JobService ONLY (Previous code duplicated insert -> Duplicate Key Error)
                    ...
                    if new_job.get("Driver_Name"):
                        try:
                            ...
                            NotificationService.send_push_to_driver(
                                new_job["Driver_Name"], "🔔 งานใหม่มาแล้ว!", f"คุณได้รับงานใหม่: {new_job['Customer_Name']}"
                            )
                        except:
                            pass
    ...
    while remaining:
        ...
        nearest = min(
            ...
            key=lambda x: haversine_distance(
                current_loc["lat"], current_loc["lon"], locations[x]["lat"], locations[x]["lon"]
            ),
        )
...
def _render_route_optimizer():
    ...
    if "origins" not in st.session_state:
        st.session_state.origins = [{"name": "Warehouse", "lat": 13.7563, "lon": 100.5018}]
    ...
    for i in range(len(st.session_state.origins)):
        ...
        with col_o2:
            o_lat = st.number_input(
                f"Lat {i+1}", value=orig["lat"], format="%.4f", key=f"orig_lat_{i}", label_visibility="collapsed"
            )
        with col_o3:
            o_lon = st.number_input(
                f"Lon {i+1}", value=orig["lon"], format="%.4f", key=f"orig_lon_{i}", label_visibility="collapsed"
            )
    ...
    for i in range(len(st.session_state.destinations) + 1):
        ...
        default_lat = (
            st.session_state.destinations[i].get("lat", 13.75 + i * 0.05)
            if i < len(st.session_state.destinations)
            else 13.75 + i * 0.05
        )
        default_lon = (
            st.session_state.destinations[i].get("lon", 100.50 + i * 0.03)
            if i < len(st.session_state.destinations)
            else 100.50 + i * 0.03
        )
        ...
        with col_d2:
            lat = st.number_input(
                f"Lat {i+1}", value=default_lat, format="%.4f", key=f"dest_lat_{i}", label_visibility="collapsed"
            )
        with col_d3:
            lon = st.number_input(
                f"Lon {i+1}", value=default_lon, format="%.4f", key=f"dest_lon_{i}", label_visibility="collapsed"
            )
    ...
    if st.button("🔄 คำนวณเส้นทางที่ดีที่สุด", type="primary"):
        ...
        if not origins or not final_dests:
            ...
            opt_dest_route, opt_dest_dist = optimize_route_nearest_neighbor(
                last_origin_idx, dest_indices, all_locations
            )
            ...
            # Savings
            savings = (
                ((original_distance - optimized_distance) / original_distance * 100) if original_distance > 0 else 0
            )
            ...
            with res_col1:
                st.markdown(
                    render_metric_card("📏 ระยะทางเดิม", f"{original_distance:.1f} km", icon="📏"),
                    unsafe_allow_html=True,
                )
            with res_col2:
                st.markdown(
            ...
            with res_col3:
                ...
                st.markdown(
                    render_metric_card("🌱 CO₂ ประหยัด", f"{co2_saved:.1f} kg", icon="🍃"), unsafe_allow_html=True
                )
